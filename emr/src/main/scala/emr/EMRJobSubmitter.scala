...
import com.amazonaws.services.elasticmapreduce.model.{ActionOnFailure, AddJobFlowStepsRequest, DescribeStepRequest, HadoopJarStepConfig, StepConfig}
...
/**
  * Submits a spark job to AWS EMR
  */
object EMRJobSubmitter extends LazyLogging {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val describeStepResult = emrClient.describeStep(describeStepRequest)
    stepState = describeStepResult.getStep.getStatus.getState
    logger.info(s"Spark job finished with state: $stepState")
  }

  private def envOrThrow(key: String) =
